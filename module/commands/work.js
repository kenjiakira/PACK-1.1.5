...
module.exports.run = async ({ api, event, Currencies, Users }) => {
    ...
    if (lastWorkTime[senderID] && currentTime - lastWorkTime[senderID] < 900000) { 
        const remainingTime = Math.ceil((900000 - (currentTime - lastWorkTime[senderID])) / 1000);
    ...
    if (job.type === "grab") {
        ...
        const response = `${userName} làm Grab và đã chở ${passengers} người và kiếm được ${grabReward} xu! 🚖💵 Tổng cộng ${totalReward} xu.`;
        api.sendMessage(response, threadID, messageID);
        return;
    }
