...
const giftDataPath = path.join(__dirname, 'cache', 'giftcodes.json');
const validGiftCodesPath = path.join(__dirname, 'cache', 'validGiftCodes.json');
...
module.exports.config = {
  ...
  description: "Nhận xu ngẫu nhiên từ GIFTCOD",
...
module.exports.run = async function({ api, event, Currencies, args }) {
  ...
  if (!validGiftCodes[giftCode]) {
    return api.sendMessage("❌ Mã gift code không hợp lệ. Ví dụ: .gift VCL", threadID, messageID);
  }
