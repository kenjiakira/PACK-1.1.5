...
const bankDataPath = path.join(__dirname, '../../includes/handle/banking.json');
...
module.exports.run = async ({ api, event, args, Currencies, Users }) => {
    ...
    if (args.length === 0) {
        ...
        return api.sendMessage(`Số tiền gửi: ${userBankData.amount.toFixed(2)} xu\nSố tiền lãi tạm tính: ${interest.toFixed(2)} xu`, threadID, messageID);
    }
    ...
    if (action === "gửi") {
        ...
        await writeBankData(bankData);
        await api.sendMessage("Bạn đã gửi " + amount + " xu vào ngân hàng. Số dư ngân hàng hiện tại của bạn là " + userBankData.amount.toFixed(2) + " xu.", threadID, messageID);
        ...
        await writeBankData(bankData);
        await api.sendMessage("Bạn đã rút " + amount + " xu từ ngân hàng. Số dư ngân hàng hiện tại của bạn là " + userBankData.amount.toFixed(2) + " xu.", threadID, messageID);
